...
# create store for number of discovered models
models_num = dcc.Store(id='discovered-models-num', storage_type='local')
# create Dropdown for conformance measures
...
# load clusters into dropdown, if clustering was successfully performed
...
# enable cluster selection, if process discovery button was clicked
...
# perform process discovery and display process model, if cluster is selected
# show average process execution feature values
...
def on_selection(clustered_ocel, avg_pe_features_list, selected_cluster):
    if selected_cluster != None:
        # get encoded OCEL of selected cluster
        ...
        # create table of avg process execution features
        if avg_pe_features_list != None:
            df_extr = pd.DataFrame(columns=["Feature", "Value"])
            # load stored feature results of cluster
...
def on_download_click(selected_cluster, clustered_ocel, filename, format, n):
    if n > 0:
        # load and encode OCEL of selected cluster
        selected_cluster_ocel = clustered_ocel[int(selected_cluster)]
        clustered_ocel_obj = pickle.loads(codecs.decode(selected_cluster_ocel.encode(), "base64"))
        # discover petri net
        ocpn = process_discovery.process_discovery_ocel_to_ocpn(clustered_ocel_obj)
...
# calculate and display conformance results for all clusters, if button is clicked
...
def on_button_click(clustered_ocels, conformance_meas, n):
    if n > 0:
        ...
        for ocel_log in clustered_ocels:
            ...
            # calculate fitness or precision
